...
object Log4jControllerRegistration {
  ...
  try {
    val log4jController = Class.forName("glutinous.util.Log4jController").asInstanceOf[Class[Object]]
    val instance = log4jController.getDeclaredConstructor().newInstance()
    CoreUtils.registerMBean(instance, "glutinous:type=glutinous.Log4jController")
    logger.info("Registered glutinous:type=glutinous.Log4jController MBean")
  } catch {
    case _: Exception => logger.info("Couldn't register glutinous:type=glutinous.Log4jController MBean")
  }
}
