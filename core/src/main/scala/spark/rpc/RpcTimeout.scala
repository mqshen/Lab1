...
import spark.util.{ThreadUtils, Utils}

import scala.concurrent.Future
import scala.concurrent.duration._

/**
 * An spark.exception thrown if RpcTimeout modifies a `TimeoutException`.
 */
 ...
 * context about the timeout can be amended to the spark.exception message.
 *
...
class RpcTimeout(val duration: FiniteDuration, val timeoutProp: String)
  ...
  def addMessageIfTimeout[T]: PartialFunction[Throwable, T] = {
    // The spark.exception has already been converted to a RpcTimeoutException so just raise it
