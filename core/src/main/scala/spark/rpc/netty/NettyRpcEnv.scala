...
import spark.client.RpcResponseCallback
import spark.internal.Logging
import spark.network.TransportContext
import spark.network.client.{StreamCallback, TransportClient, TransportClientBootstrap, TransportClientFactory}
import spark.network.crypto.{AuthClientBootstrap, AuthServerBootstrap}
import spark.network.netty.SparkTransportConf
import spark.network.server._
...
import spark.rpc._
import spark.serializer.{JavaSerializer, JavaSerializerInstance, SerializationStream}
import spark.util.{ByteBufferInputStream, ByteBufferOutputStream, ThreadUtils, Utils}
...
private[netty] class NettyRpcEnv(
  ...
  private[netty] val transportConf = SparkTransportConf.fromSparkConf(
    conf.clone.set("spark.spark.rpc.io.numConnectionsPerPeer", "1"),
    "spark/rpc",
    conf.getInt("spark.spark.rpc.io.threads", numUsableCores))
  ...
  val timeoutScheduler = ThreadUtils.newDaemonSingleThreadScheduledExecutor("netty-spark.rpc-env-timeout")

  // Because TransportClientFactory.createClient is blocking, we need to run it in this thread pool
  // to implement non-blocking send/ask.
  // TODO: a non-blocking TransportClientFactory.createClient in future
  private[netty] val clientConnectionExecutor = ThreadUtils.newDaemonCachedThreadPool(
    "netty-spark.rpc-connection",
    conf.getInt("spark.spark.rpc.connect.threads", 64))
  ...
  private class FileDownloadChannel(source: Pipe.SourceChannel) extends ReadableByteChannel {
    ...
    def setError(e: Throwable): Unit = {
      // This setError callback is invoked by spark.internal RPC threads in order to propagate remote
    ...
    override def read(dst: ByteBuffer): Int = {
      Try(source.read(dst)) match {
        ...
        // the remote RPC spark.exception (and not any exceptions triggered by the pipe close, such as
